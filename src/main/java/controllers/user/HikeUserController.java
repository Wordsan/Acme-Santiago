...
public class HikeUserController extends AbstractController {
	...
	public ModelAndView edit(@RequestParam(required = true) final int hikeId, final RedirectAttributes redirectAttrs) {
		ModelAndView view;
		Hike hike;

		try {
			hike = this.hikeService.findOne(hikeId);
			Assert.isTrue(this.userService.getUserByUserAccountId(LoginService.getPrincipal().getId()).equals(hike.getRoute().getCreator()));
			view = this.createEditModelAndView(hike);
	...
	public ModelAndView edit(Hike hike, final BindingResult binding, final RedirectAttributes redirectAttrs) {
		ModelAndView view;
		...
		try {
			...
			if (binding.hasErrors()) {
				...
				hike.getRoute().getComposedHikes().add(hike);
				this.routeService.save(hike.getRoute());
	...
	public ModelAndView delete(Hike hike, final BindingResult binding, final RedirectAttributes redirectAttrs) {
		ModelAndView view;
	...
	private ModelAndView createEditModelAndView(final Hike hike) {
		return this.createEditModelAndView(hike, null);
	}

	private ModelAndView createEditModelAndView(final Hike hike, final String message) {
		ModelAndView view;
